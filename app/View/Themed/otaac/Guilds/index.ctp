						...
						<?php if($this->Session->check('Account')) { ?>
							<td class="text-right">
								<?php if($this->Session->read('Account.name') == $guild['Player']['name']){ // Se quem estiver logado for o dono da guilda ?>
									<a href="<?php echo url; ?>guilds/manage/<?php echo $guild['Guild']['id']; ?>" type="button" class="btn btn-default btn-xs" title="Gerenciar"><span class="glyphicon glyphicon-wrench"></span></a>
